...
export class GenerateReport {
  ...
  createTableCR(reportCRDetails) {
    if (reportCRDetails.length > 0) {
      ...
      for (let cR of reportCRDetails) {
        ...
        table.getCell(i, 3).addContent(new Paragraph(cR.status).heading5().center());
        i++;
      }
      return table;
    }
  ...
  createTableActivities(reportActivityDetails) {
    if (reportActivityDetails.length > 0) {
      ...
      for (let rAD of reportActivityDetails) {
        table1.getCell(i, 0).addContent(new Paragraph(i.toString()).heading5().center());
        table1.getCell(i, 1).addContent(new Paragraph(rAD.milestones).heading5().left());
        table1.getCell(i, 2).addContent(new Paragraph(rAD.eta).heading5().center());
        i++;
      }
      return table1;
    }
  ...
  createBillDetails(reportSummery) {
    ...
    table2.getCell(9, 0).addContent(new Paragraph("Total Off-Shore Hours Utilized").heading3().left());
    table2.getCell(9, 1).addContent(new Paragraph(this.offShoreHoursUtilized.toString()).heading5());
    table2.getCell(10, 0).addContent(new Paragraph("Total Off-Shore Hours Remaining").heading3().left());
    table2.getCell(10, 1).addContent(new Paragraph(this.offShoreHoursRemaining.toString()).heading5());
